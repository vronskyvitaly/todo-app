        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 min-w-0 space-y-1">
              <label className="block text-xs font-medium text-slate-400">Deadline</label>
              <div
                className="relative cursor-pointer"
                onClick={(e) => {
                  const input = e.currentTarget.querySelector<HTMLInputElement>('input[type="date"]');
                  try { input?.showPicker?.(); } catch { /* fallback: native tap on mobile */ }
                }}
              >
