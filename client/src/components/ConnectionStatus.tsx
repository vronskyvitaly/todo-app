...
export default function ConnectionStatus() {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span
        className={`inline-block h-2.5 w-2.5 flex-shrink-0 rounded-full transition-colors duration-300 shadow-sm
      ...
      {error && !isConnecting && (
        <span className="hidden sm:inline text-xs text-red-400 bg-red-950/40 px-2 py-0.5 rounded-full">
          {error}
        </span>
      )}
    </div>
